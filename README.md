...
curl -fsSL https://raw.githubusercontent.com/CAPSTONEID/sins-clco/main/sins-skill-all-install.sh \
  | sed 's|~/.claude/skills|~/.codex/skills|g' | bash
```
...
cp /tmp/sins-clco/skill-list/*.skill ~/.codex/skills/
rm -rf /tmp/sins-clco
echo "✅ Codex 설치 완료"
```
...
```
@~/.codex/skills/sins-marketing-team.skill 위 내용대로 실행해줘
```
